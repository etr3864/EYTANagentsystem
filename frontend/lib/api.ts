...
export async function pauseConversation(convId: number): Promise<void> {
  const res = await fetch(`${API_URL}/api/conversations/${convId}/pause`, { method: 'POST' });
...
export async function resumeConversation(convId: number): Promise<void> {
  const res = await fetch(`${API_URL}/api/conversations/${convId}/resume`, { method: 'POST' });
...
export async function sendMessage(convId: number, text: string): Promise<{ status: string; message_id: number }> {
  const res = await fetch(`${API_URL}/api/conversations/${convId}/send`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export async function getDbConversations(): Promise<DbConversation[]> {
  const res = await fetch(`${API_URL}/api/db/conversations`);
  if (!res.ok) throw new Error('Failed to fetch conversations');
  return res.json();
}

export async function getDbMessages(limit = 100): Promise<DbMessage[]> {
  const res = await fetch(`${API_URL}/api/db/messages?limit=${limit}`);
  if (!res.ok) throw new Error('Failed to fetch messages');
  return res.json();
}

export async function deleteDbMessage(msgId: number): Promise<void> {
  const res = await fetch(`${API_URL}/api/db/messages/${msgId}`, { method: 'DELETE' });
  if (!res.ok) throw new Error('Failed to delete message');
}

export async function getUsageStats(): Promise<UsageStats[]> {
  const res = await fetch(`${API_URL}/api/db/usage`);
...
export async function updateAgentMedia(
  ...
  const res = await fetch(`${API_URL}/api/agents/${agentId}/media/${mediaId}`, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(data),
  });
...
export async function deleteAgentMedia(agentId: number, mediaId: number): Promise<void> {
  const res = await fetch(`${API_URL}/api/agents/${agentId}/media/${mediaId}`, { method: 'DELETE' });
  if (!res.ok) throw new Error('Failed to delete media');
}
...
export async function getDocuments(agentId: number): Promise<Document[]> {
  const res = await fetch(`${API_URL}/api/agents/${agentId}/knowledge/documents`);
...
export async function deleteDocument(agentId: number, docId: number): Promise<void> {
  const res = await fetch(`${API_URL}/api/agents/${agentId}/knowledge/documents/${docId}`, { method: 'DELETE' });
  if (!res.ok) throw new Error('Failed to delete document');
}

export async function getDataTables(agentId: number): Promise<DataTable[]> {
  const res = await fetch(`${API_URL}/api/agents/${agentId}/knowledge/tables`);
...
export async function deleteDataTable(agentId: number, tableId: number): Promise<void> {
  const res = await fetch(`${API_URL}/api/agents/${agentId}/knowledge/tables/${tableId}`, { method: 'DELETE' });
