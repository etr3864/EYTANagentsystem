...
function SettingsPage() {
  ...
  async function handleCreate() {
    ...
    setError('');
    try {
      const row = await createMcpToken(name.trim() || 'Cursor');
      setCreated(row);
  ...
  async function handleCopy(kind: 'token' | 'json', value: string) {
  ...
  return (
    <div className={`${BELOW_NAV_CLASS} max-w-3xl mx-auto px-3 md:px-6 py-6 space-y-6`}>
      <div>
        ...
        <p className="text-slate-400 mt-1">
          טוקני MCP למנהל ראשי. מי שיש לו טוקן פעיל פועל בשמך — דשבורד, API ו־MCP. אין תפוגה.
          השהה כדי לחתוך גישה זמנית, או מחק כדי לבטל לצמיתות.
        </p>
      </div>
      ...
      <Card>
        ...
        <p className="text-slate-300 text-sm leading-relaxed">
          MCP מחבר את Cursor לאופטיב. אחרי החיבור אפשר לבנות ולתפעל סוכנים, לקוחות ועובדים —
          ולקרוא עלויות וביצועים כמו בדשבורד מנהלים, לפי סוכן.
        </p>
        <p className="text-slate-400 text-sm mt-3">
          שליחת הודעה ללקוח, מחיקות ואיפוס סיסמה דורשים אישור מפורש מהמודל (`confirm=true`).
          חיבור WhatsApp / יומן גוגל נפתח בדפדפן — ה-MCP רק מחזיר קישור.
        </p>
      </Card>
      ...
      <Card>
        ...
        {created && (
          <div className="mb-5 rounded-lg border border-amber-500/30 bg-amber-500/10 p-4 space-y-3">
            <p className="text-amber-100 text-sm font-medium">
              הטוקן מוצג פעם אחת. העתק עכשיו — אחרי רענון לא נראה אותו שוב.
            </p>
        ...
        <div className="flex flex-col sm:flex-row gap-3 sm:items-end mb-5">
          <div className="flex-1">
            <Input
              ...
              placeholder="Cursor / מק או / גיבוי"
              maxLength={80}
            />
          </div>
        ...
        {loading ? (
          ...
          <p className="text-slate-400 text-sm">אין טוקנים עדיין. צור אחד כדי לחבר את Cursor.</p>
        ) : (
      ...
      <Card>
        <h2 className="text-lg font-medium text-white mb-3">חיבור ל-Cursor</h2>
        <ol className="list-decimal pr-5 space-y-2 text-sm text-slate-300 leading-relaxed">
          <li>צור טוקן למעלה והעתק אותו.</li>
          <li>
            ב-Cursor: <span className="text-white">Settings → MCP → New MCP Server</span>
            {' '}(או קובץ <code className="text-purple-300">~/.cursor/mcp.json</code>).
          </li>
          <li>הדבק את ה-JSON. שמור והפעל את השרת <span className="text-white">optive</span>.</li>
          <li>
            בצ׳אט חדש תראה כלים כמו <code className="text-purple-300">list_agents</code>,{' '}
            <code className="text-purple-300">list_agent_costs</code> ו-<code className="text-purple-300">get_agent_performance</code>.
          </li>
        </ol>
        <p className="text-slate-500 text-xs mt-3">
          שים את הקובץ בהגדרות המשתמש, לא בריפו משותף. מי שיש לו את הטוקן פועל בשמך.
        </p>
