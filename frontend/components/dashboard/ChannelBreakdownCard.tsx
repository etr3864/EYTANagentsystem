...
export function ChannelBreakdownCard({ fromDate, toDate, agentId, compact, endpoint }: ChannelBreakdownCardProps) {
  ...
  return (
    <div className={compact ? '' : 'bg-[var(--bg-secondary)] border border-[var(--edge)] rounded-2xl p-5'}>
      {!compact && <h3 className="text-sm font-semibold text-[var(--ink)] mb-4">פילוח ערוצים</h3>}
      ...
      {loading ? (
        ...
        <p className="text-[var(--text-muted)] text-sm text-center py-3">אין נתוני ערוצים לתקופה זו</p>
      ) : (
          ...
          {stats.map(stat => {
            ...
            return (
              <div key={stat.channel_type}>
                ...
                <div className="h-1.5 bg-[var(--glass-2)] rounded-full overflow-hidden">
                  <div
