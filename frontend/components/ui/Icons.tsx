/**
 * Shared SVG icons used across the application.
 * All icons accept optional className for size/color overrides.
 */
...
export function VoiceIcon({ className = 'w-4 h-4' }: IconProps) {
  return (
...
export function MetaSmallIcon({ className = 'w-3 h-3' }: IconProps) {
  return (
...
export function WaSenderSmallIcon({ className = 'w-3 h-3' }: IconProps) {
  return (
    <svg className={className} viewBox="0 0 24 24" fill="currentColor">
...
// ── Channel Icons (official brand logos) ─────────────────────────────────────
...
function InstagramLogo({ size = 16 }: { size: number }) {
  ...
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none">
      ...
      <rect x="3" y="3" width="18" height="18" rx="4.5" stroke="white" strokeWidth="1.8" fill="none" />
...
export function ChannelIcon({ channelType, size = 16, className }: ChannelIconProps) {
  const logo = (() => {
    switch (channelType) {
      ...
      case 'whatsapp_meta':
      case 'whatsapp_wasender':
      default: return <WhatsAppLogo size={size} />;
    }
  })();
