...
export function FieldList({ fields, onChange }: FieldListProps) {
  ...
  return (
    <div className="space-y-3 rounded-lg border border-[var(--edge)] bg-[var(--glass)] p-3">
      <div>
        <p className="text-sm font-medium text-[var(--ink)]">שדות שיישלחו</p>
      ...
      {fields.length === 0 && (
        <p className="text-xs text-[var(--text-muted)]">אין שדות עדיין. הפק מההנחיה או הוסף ידנית.</p>
      )}
      {fields.map((field, index) => (
        <div key={index} className="grid gap-2 rounded-lg border border-[var(--edge)] bg-[var(--bg)]/60 p-3">
