...
type Section = 'images' | 'videos' | 'settings';
...
export function MediaTab({
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>, mediaType: 'image' | 'video') => {
    ...
    const allowed = mediaType === 'image' ? allowedImage : allowedVideo;
    ...
    for (let i = 0; i < validFiles.length; i++) {
      ...
      // Get name from filename (remove extension) - used for videos and display
      ...
      setUploadStatus(mediaType === 'image' ? 'מעלה תמונה...' : `מעלה ${fileBaseName}...`);
      
      const wasCompressed = fileToUpload !== file;
      
      try {
        await onUpload(fileToUpload, {
          // For images: don't send name - let AI generate it
          // For videos: use filename
          name: mediaType === 'video' ? fileBaseName : undefined,
          media_type: mediaType,
          original_size: wasCompressed ? file.size : undefined
        }, (progress) => {
          setUploadProgress(progress);
          if (mediaType === 'image') {
            // Images: upload -> analyze -> embedding
            if (progress < 50) setUploadStatus('מעלה תמונה...');
            else if (progress < 100) setUploadStatus('מנתח תמונה...');
          } else {
            // Videos: upload -> embedding
  ...
  const saveEdit = async (mediaId: number) => {
    try {
            ...
            {isEditing ? (
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  <Button size="sm" variant="primary" onClick={() => saveEdit(item.id)}>שמור</Button>
