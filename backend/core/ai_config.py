...
USER_TOOLS = [
    ...
    {
        "name": "send_media",
        "description": "שלח תמונה או וידאו ללקוח. השתמש בכלי זה כשהלקוח מבקש לראות תמונה, דוגמה, קטלוג, או כשרלוונטי להציג משהו ויזואלי.",
        "input_schema": {
            "type": "object",
            "properties": {
                "media_id": {"type": "integer", "description": "מזהה המדיה לשליחה (מהרשימה שקיבלת)"},
                "caption": {"type": "string", "description": "כיתוב לתמונה/וידאו (אופציונלי - אם לא צוין ישתמש בברירת מחדל)"}
            },
            "required": ["media_id"]
        }
    },
    {
        "name": "search_media",
        "description": "חפש תמונה או וידאו במאגר המדיה לפי תיאור. השתמש כשיש הרבה מדיה ואתה צריך למצוא משהו ספציפי.",
        "input_schema": {
            "type": "object",
            "properties": {
...
---
הנחיות מערכת:
- כשאתה לומד מידע חדש על המשתמש (שם, מגדר, תחום עסק, הערות), השתמש בכלי update_user_info.
...
כלי מדיה (אם יש מדיה זמינה):
- send_media: שלח תמונה או וידאו ללקוח (דורש media_id מהרשימה)
- search_media: חפש מדיה לפי תיאור (רק כשיש הרבה פריטים ואתה צריך למצוא משהו ספציפי)
- כשהלקוח מבקש לראות משהו, דוגמה, תמונה, קטלוג - חובה להשתמש ב-send_media!
- אל תתאר תמונה במילים - שלח אותה עם send_media.
"""
